...
add_shortcode( 'stray-incidents-form', 'stray_incidents_form' );
	
function stray_incidents_form() {
	...
	$disabled = ( $post_id == '' || current_user_can( "publish_posts" ) ? "" : "disabled" );
	...
	<?php else: ?>
		...
		if ($revisions && current_user_can('edit_others_posts') && $post_id) :?>
			<hr>
			<h4>Ιστορικό επεξεργασίας περιστατικού</h4>
			<table class="table table-striped">
				<th>Ημερομηνία</th>
				<th>Χρήστης</th>
				<?php foreach ($revisions as $revision): ?>
				<tr>
					<td><?php echo $revision->post_date; ?></td>
					<td><?php echo get_userdata($revision->post_author)->user_email; ?></td>
				</tr>	
				<?php endforeach; ?>
			</table>
			<hr>
		...
		<form id="stray-incidents-form" autocomplete="off">
			<div class="form-group row">
				<label class="col-sm-4 col-form-label" for="date">Κωδικός περιστατικού</label> 
				<div class="col-sm-8">
					<input <?php echo $disabled; ?> id="post_title" name="post_title" type="text" value="<?php echo $post_id ? $post_title : ""; ?>" class="form-control" placeholder="Συμπληρώστε μόνο αν θέλετε να θέσετε συγκεκριμένο κωδικό περιστατικού">
				</div>
			</div>
			
			<div class="form-group row">
				<label class="col-sm-4 col-form-label" for="date">Περιστατικά που συνδέονται (χωρισμένα με κόμμα)</label> 
				<div class="col-sm-8">
					<input <?php echo $disabled; ?> id="connected_incidents" name="connected_incidents" type="text" value="<?php echo $meta['connected_incidents']; ?>" class="form-control">
					<?php if ($meta['connected_incidents']): ?>
						<span>Γρήγοροι σύνδεσμοι: </span>
					<?php endif; ?>
					<?php foreach (explode(',', $meta['connected_incidents']) as $connected_incident):?>
						<a href="/?post_id=<?php echo $connected_incident; ?>"><?php echo $connected_incident; ?></a>
					<?php endforeach; ?>
				</div>
			</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-8">
					<select <?php echo $disabled; ?> id="area" name="area" class="custom-select" required>
						<option></option>
						<? $areas = unserialize(get_option('stray_incidents_areas'));
						foreach ($areas as $area): ?>
							<option value="<?php echo $area;?>" <?php if ($meta['area'] == $area) echo "selected";?>><?php echo $area;?></option>
						<?php endforeach; ?>
					  </select> 
				</div>
			</div>
			...
			<input type="hidden" id="security" name="security" value="<?php echo wp_create_nonce('stray-incidents-form'); ?>"/>
		</form>
...
add_action( 'wp_ajax_stray_incidents_submission', 'stray_incidents_submission' );
function stray_incidents_submission() {
	...
	if ($_FILES) {
		...
		$_POST['connected_incidents'] = implode(',', array_map('trim', explode(',', $_POST['connected_incidents'])));
...
add_action( 'wp_ajax_stray_incidents_delete', 'stray_incidents_delete' );
function stray_incidents_delete() {
...
add_action( 'wp_ajax_stray_incidents_delete_file', 'stray_incidents_delete_file' );
function stray_incidents_delete_file() {
